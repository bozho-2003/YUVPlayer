...
// format name
const char * CFormater_YUYV::getName(){
	return YUV_FORMATER_NAME;
}
// other information
int  CFormater_YUYV::getModuleInfo(Module_Info *pInfo){
...
int  CFormater_YUYV::simpleConfig(int val, unsigned int val2, const char *str){
	return 0;
}
int  CFormater_YUYV::selfConfig(){
	return 0;
}

int  CFormater_YUYV::queryRuntimeInfo(char *infoBuf, int bufsz){
	return 0;
}

// 设置源数据格式, width, height, field
int  CFormater_YUYV::setParams(YUV_Params *params){
	if(params == NULL)
		return E_BAD_ARG;
...
int  CFormater_YUYV::update(void *pErr){
...
int  CFormater_YUYV::getOutInfo(RawImage_Info *out){
	if(out == NULL)
		return E_BAD_ARG;
...
int  CFormater_YUYV::getMinBlockSize(){
	return m_block_size;
}
int  CFormater_YUYV::getMinBlockFrameNum(){
	return m_frame_num;
}

// 获取buffer, buf大小应为minBlockSize
int  CFormater_YUYV::getBuffer(unsigned char **ppBuf){
	if(ppBuf == NULL)
		return E_BAD_ARG;
	*ppBuf = NULL;
...
int  CFormater_YUYV::putData(unsigned char *pBuf){
	if(pBuf != m_buf)
		return E_BAD_ARG;
	m_index = 0;
	return E_OK;
...
int  CFormater_YUYV::getFrameIndex(){
	return m_index;
}
int  CFormater_YUYV::setFrameIndex(int index){
...
int  CFormater_YUYV::getFrame(int offset, unsigned char *pBuf, int bufsz){
	int index = m_index;
	if(index < 0)
		return E_NO_INIT;
	if(offset > 0) index++;
...
int  CFormater_YUYV::toYourFormat(RawImage_Info *src_info, const unsigned char *src_buf, int src_bufsz, unsigned char *dst_buf, int *p_dst_bufsz){
	if(src_info == NULL)
		return -1;
