...
#define YUV_FORMATER_NAME  "YV16"

#define MODULE_VERSION    "0.0.1"
#define MODULE_AUTHOR     "netjunegg@gmail.com"
#define MODULE_COMMENT    "yv16, 与YUV422的区别是UV位置相反, yvu, 4:2:2"


void CFormater_YV16::_init(){
	m_buf = NULL;
	m_bufsz = 0;
...
// format name
const char * CFormater_YV16::getName(){
	return YUV_FORMATER_NAME;
}
// other information
int  CFormater_YV16::getModuleInfo(Module_Info *pInfo){
...
int  CFormater_YV16::simpleConfig(int val, unsigned int val2, const char *str){
	return 0;
}
int  CFormater_YV16::selfConfig(){
	return 0;
}

int  CFormater_YV16::queryRuntimeInfo(char *infoBuf, int bufsz){
	return 0;
}

// 设置源数据格式, width, height, field
int  CFormater_YV16::setParams(YUV_Params *params){
	if(params == NULL)
		return E_BAD_ARG;
...
int  CFormater_YV16::update(void *pErr){
...
int  CFormater_YV16::getOutInfo(RawImage_Info *out){
	if(out == NULL)
		return E_BAD_ARG;
...
int  CFormater_YV16::getMinBlockSize(){
	return m_block_size;
}
int  CFormater_YV16::getMinBlockFrameNum(){
	return m_frame_num;
}

// 获取buffer, buf大小应为minBlockSize
int  CFormater_YV16::getBuffer(unsigned char **ppBuf){
	if(ppBuf == NULL)
		return E_BAD_ARG;
	*ppBuf = NULL;
...
int  CFormater_YV16::putData(unsigned char *pBuf){
	if(pBuf != m_buf)
		return E_BAD_ARG;
	m_index = 0;
	return E_OK;
...
int  CFormater_YV16::getFrameIndex(){
	return m_index;
}
int  CFormater_YV16::setFrameIndex(int index){
...
int  CFormater_YV16::getFrame(int offset, unsigned char *pBuf, int bufsz){
	int index = m_index;
	if(index < 0)
		return E_NO_INIT;
	if(offset > 0) index++;
...
int  CFormater_YV16::toYourFormat(RawImage_Info *src_info, const unsigned char *src_buf, int src_bufsz, unsigned char *dst_buf, int *p_dst_bufsz){
	if(src_info == NULL)
		return -1;
...
int formater_yv16_get_interface(void **ppIntf){
	if(ppIntf == NULL)
		return -1;
	*ppIntf = new CFormater_YV16;
	return (ppIntf != NULL);
}
