...
#define YUV_FORMATER_NAME  "I422(yuv422)"

#define MODULE_VERSION    "0.0.1"
#define MODULE_AUTHOR     "netjunegg@gmail.com"
#define MODULE_COMMENT    "i422, yuv, 4:2:2"


void CFormater_I422::_init(){
	m_buf = NULL;
	m_bufsz = 0;
...
// format name
const char * CFormater_I422::getName(){
	return YUV_FORMATER_NAME;
}
// other information
int  CFormater_I422::getModuleInfo(Module_Info *pInfo){
...
int  CFormater_I422::simpleConfig(int val, unsigned int val2, const char *str){
	return 0;
}
int  CFormater_I422::selfConfig(){
	return 0;
}

int  CFormater_I422::queryRuntimeInfo(char *infoBuf, int bufsz){
	return 0;
}

// 设置源数据格式, width, height, field
int  CFormater_I422::setParams(YUV_Params *params){
	if(params == NULL)
		return E_BAD_ARG;
...
int  CFormater_I422::update(void *pErr){
...
int  CFormater_I422::getOutInfo(RawImage_Info *out){
	if(out == NULL)
		return E_BAD_ARG;
...
int  CFormater_I422::getMinBlockSize(){
	return m_block_size;
}
int  CFormater_I422::getMinBlockFrameNum(){
	return m_frame_num;
}

// 获取buffer, buf大小应为minBlockSize
int  CFormater_I422::getBuffer(unsigned char **ppBuf){
	if(ppBuf == NULL)
		return E_BAD_ARG;
	*ppBuf = NULL;
...
int  CFormater_I422::putData(unsigned char *pBuf){
	if(pBuf != m_buf)
		return E_BAD_ARG;
	m_index = 0;
	return E_OK;
...
int  CFormater_I422::getFrameIndex(){
	return m_index;
}
int  CFormater_I422::setFrameIndex(int index){
...
int  CFormater_I422::getFrame(int offset, unsigned char *pBuf, int bufsz){
	int index = m_index;
	if(index < 0)
		return E_NO_INIT;
	if(offset > 0) index++;
...
int  CFormater_I422::toYourFormat(RawImage_Info *src_info, const unsigned char *src_buf, int src_bufsz, unsigned char *dst_buf, int *p_dst_bufsz){
	if(src_info == NULL)
		return -1;
