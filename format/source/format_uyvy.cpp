...
#define YUV_FORMATER_NAME  "UYVY"

#define MODULE_VERSION    "0.0.1"
#define MODULE_AUTHOR     "netjunegg@gmail.com"
#define MODULE_COMMENT    "uyvy, uyvy"


void CFormater_UYVY::_init(){
	m_buf = NULL;
	m_bufsz = 0;
...
// format name
const char * CFormater_UYVY::getName(){
	return YUV_FORMATER_NAME;
}
// other information
int  CFormater_UYVY::getModuleInfo(Module_Info *pInfo){
...
int  CFormater_UYVY::simpleConfig(int val, unsigned int val2, const char *str){
	return 0;
}
int  CFormater_UYVY::selfConfig(){
	return 0;
}

int  CFormater_UYVY::queryRuntimeInfo(char *infoBuf, int bufsz){
	return 0;
}

// 设置源数据格式, width, height, field
int  CFormater_UYVY::setParams(YUV_Params *params){
	if(params == NULL)
		return E_BAD_ARG;
...
int  CFormater_UYVY::update(void *pErr){
...
int  CFormater_UYVY::getOutInfo(RawImage_Info *out){
	if(out == NULL)
		return E_BAD_ARG;
...
int  CFormater_UYVY::getMinBlockSize(){
	return m_block_size;
}
int  CFormater_UYVY::getMinBlockFrameNum(){
	return m_frame_num;
}

// 获取buffer, buf大小应为minBlockSize
int  CFormater_UYVY::getBuffer(unsigned char **ppBuf){
	if(ppBuf == NULL)
		return E_BAD_ARG;
	*ppBuf = NULL;
...
int  CFormater_UYVY::putData(unsigned char *pBuf){
	if(pBuf != m_buf)
		return E_BAD_ARG;
	m_index = 0;
	return E_OK;
...
int  CFormater_UYVY::getFrameIndex(){
	return m_index;
}
int  CFormater_UYVY::setFrameIndex(int index){
...
int  CFormater_UYVY::getFrame(int offset, unsigned char *pBuf, int bufsz){
	int index = m_index;
	if(index < 0)
		return E_NO_INIT;
	if(offset > 0) index++;
...
int  CFormater_UYVY::toYourFormat(RawImage_Info *src_info, const unsigned char *src_buf, int src_bufsz, unsigned char *dst_buf, int *p_dst_bufsz){
	if(src_info == NULL)
		return -1;
